...
export class ReservationListBookHourComponent implements OnInit {
  ...
  private InitDropdownFilters() {
    this.data.filteredOptions = this.data.bookingForm.get('name').valueChanges.pipe(
      startWith(""),
      map((value) => {
        const name = typeof value === "string" ? value : (value as any)?.name;
        return name ? this._filter(name as string) : this.data.options.slice();
      })
    );

    this.data.filteredPhoneOptions = this.data.bookingForm.get('phone').valueChanges.pipe(
      startWith(""),
      map((value) => {
        const phone = typeof value === "string" ? value : (value as any)?.phone;
        return phone
    ...
    this.data.filteredEmailOptions = this.data.bookingForm.get('email').valueChanges.pipe(
      startWith(""),
      map((value) => {
